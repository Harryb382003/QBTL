...
my $stored = $db->store_API_torrents_info(
    $dbh,
    [
      {
       hash          => $first_hash,
...
is(
    $dbh->selectall_arrayref(
      q{SELECT infohash, discovered_on, discovered_by FROM torrents ORDER BY infohash},
      {Slice => {}},
    ),
    [
      {
...
my $payload = decode_json(
    $dbh->selectrow_array(
      q{SELECT payload_json FROM API_torrents_info WHERE infohash = ?},
      undef,
      $first_hash,
    )
);

is( $payload->{hash}, $first_hash, 'original API hash key retained in payload' );
is( $payload->{future_key}, 'retained only in the complete payload',
...
my $updated = $db->store_API_torrents_info(
    $dbh,
    [
      {
       hash         => $first_hash,
...
my $error = dies {
  $db->store_API_torrents_info(
    $dbh,
    [
      { hash => 'fedcba9876543210fedcba9876543210fedcba98', name => 'valid' },
      { name => 'missing hash' },
    ],
    $updated_on + 60,
  );
